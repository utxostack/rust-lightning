...
fn do_test_async_raa_peer_disconnect(
	...
	if test_case == UnblockSignerAcrossDisconnectCase::BeforeMonitorRestored {
		...
		let (outpoint, latest_update, _) = dst
			.chain_monitor
...
fn do_test_async_commitment_signature_ordering(monitor_update_failure: bool) {
	...
	nodes[0]
		.node
		.handle_commitment_signed(&nodes[1].node.get_our_node_id(), &bs_second_commitment_signed);
	...
	if extra_closing_signed {
		let node_1_closing_signed_2_bad = {
			...
			let chan = chan_lock
				.channel_by_id
				.get_mut(&chan_1.2)
				.map(|phase| phase.context_mut())
				.unwrap();

			let signer = chan.get_mut_signer().as_mut_ecdsa().unwrap();
			let signature =
				signer.sign_closing_transaction(&closing_tx_2, &Secp256k1::new()).unwrap();
			node_1_closing_signed_2.signature = signature;
			node_1_closing_signed_2
		};
		nodes[0]
			.node
			.handle_closing_signed(&nodes[1].node.get_our_node_id(), &node_1_closing_signed_2_bad);
