...
use crate::events::{
	ClosureReason, Event, FundingInfo, HTLCDestination, MessageSendEvent,
...
use crate::ln::channel::{
	get_holder_selected_channel_reserve_satoshis, ChannelPhase, InboundV1Channel,
	OutboundV1Channel, COINBASE_MATURITY, CONCURRENT_INBOUND_HTLC_FEE_BUFFER,
...
fn channel_monitor_network_test() {
	...
	let chan_3_mon = nodes[3]
		.chain_monitor
		.chain_monitor
		.remove_monitor(&OutPoint { txid: chan_3.3.compute_txid(), index: 0 });
...
fn do_test_forming_justice_tx_from_monitor_updates(broadcast_initial_commitment: bool) {
	...
	let justice_tx =
		persisters[1].justice_tx(funding_txo, &revoked_commitment_tx.compute_txid()).unwrap();
...
fn claim_htlc_outputs_single_tx() {
...

	{
		...
		node_txn
			.retain(|tx| tx.input[0].previous_output.txid == revoked_local_txn[0].compute_txid());
...
fn test_duplicate_payment_hash_one_failure_one_success() {
	...
	let htlc_timeout_tx;
	{
		...
		if node_txn.len() > 2 {
			...
			htlc_timeout_tx = if node_txn[2].output[0].value.to_sat() < 900 {
				node_txn[2].clone()
			} else {
				node_txn[0].clone()
			};
		} else {
			htlc_timeout_tx = if node_txn[0].output[0].value.to_sat() < 900 {
				node_txn[1].clone()
			} else {
				node_txn[0].clone()
			};
...
fn test_bump_txn_sanitize_tracking_maps() {
	...
	connect_blocks(&nodes[0], ANTI_REORG_DELAY - 1);
	{
		let monitor = nodes[0]
			.chain_monitor
			.chain_monitor
			.get_monitor(OutPoint { txid: chan.3.compute_txid(), index: 0 })
			.unwrap();
...
fn do_test_max_dust_htlc_exposure(
	...
	let mut commitment_tx_cost = commit_tx_fee_msat(
		initial_feerate - 253,
		nondust_htlc_count_in_limit,
		&ChannelTypeFeatures::empty(),
	);
	commitment_tx_cost += if on_holder_tx {
		...
		/ 1000 * nondust_htlc_count_in_limit;
	{
		let mut feerate_lock = chanmon_cfgs[0].fee_estimator.sat_per_kw.lock().unwrap();
	...
	config.channel_config.max_dust_htlc_exposure = if multiplier_dust_limit {
		...
		MaxDustHTLCExposure::FixedLimitMsat(5_000_000 + commitment_tx_cost)
	};
	let node_cfgs = create_node_cfgs(2, &chanmon_cfgs);
...
fn test_nondust_htlc_fees_are_dust() {
	...
	let commitment_tx_per_htlc_cost = htlc_success_tx_weight(&ChannelTypeFeatures::empty()) * 253;
	let max_htlcs_remaining = dust_limit * 2 / commitment_tx_per_htlc_cost;
	assert!(
		max_htlcs_remaining < 30,
		"We should be able to fill our dust limit without too many HTLCs"
	);
