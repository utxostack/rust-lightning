...
use crate::ln::channelmanager::{
	self, Bolt12PaymentError, PaymentId, RecentPaymentDetails, Retry,
...
fn extract_offer_nonce<'a, 'b, 'c>(node: &Node<'a, 'b, 'c>, message: &OnionMessage) -> Nonce {
	match node.onion_messenger.peel_onion_message(message) {
		Ok(PeeledOnion::Receive(
			_,
			Some(MessageContext::Offers(OffersContext::InvoiceRequest { nonce })),
			_,
		)) => nonce,
		Ok(PeeledOnion::Receive(_, context, _)) => {
...
fn extract_invoice_request<'a, 'b, 'c>(
	...
	match node.onion_messenger.peel_onion_message(message) {
		Ok(PeeledOnion::Receive(message, _, reply_path)) => match message {
			...
			#[cfg(async_payments)]
			ParsedOnionMessageContents::AsyncPayments(message) => {
				panic!("Unexpected async payments message: {:?}", message)
			},
...
fn extract_invoice<'a, 'b, 'c>(
	node: &Node<'a, 'b, 'c>, message: &OnionMessage,
) -> (Bolt12Invoice, Option<BlindedMessagePath>) {
	match node.onion_messenger.peel_onion_message(message) {
		Ok(PeeledOnion::Receive(message, _, reply_path)) => match message {
			ParsedOnionMessageContents::Offers(offers_message) => match offers_message {
				OffersMessage::InvoiceRequest(invoice_request) => {
...
fn send_invoice_for_refund_with_distinct_reply_path() {
	...
	*node_cfgs[1].override_init_features.borrow_mut() = Some(features);
	...
	let (_, reply_path) = extract_invoice(alice, &onion_message);
	assert_eq!(
		reply_path.unwrap().introduction_node(),
		&IntroductionNode::NodeId(nodes[6].node.get_our_node_id())
	);
}
...
fn fails_creating_refund_with_duplicate_payment_id() {
	...
	assert!(nodes[0]
		.node
		.create_refund_builder(10_000, absolute_expiry, payment_id, Retry::Attempts(0), None)
		.is_ok());
